# TLS resources.  The resources in this file are entirely for generating self-signed certs for trial purposes.
...
resource "tls_cert_request" "server" {
  ...
  dns_names = [
    var.vault_lb_hostname != null? var.vault_lb_hostname : aws_lb.vault_lb.dns_name,
    "localhost",
  ]
#  key_algorithm = "RSA"
  ip_addresses = [
...
# A locally signed certificate fro vault
...
# Assemble the cert and key data into a json blob that can be stored in SecretsManager and parsed by userdata when each Vault instance starts up.
locals {
  tls_data = {
...
# encode the generated key material
locals {
